...
namespace Utilities
{
    public static class MFAService
    {
        ...
        public static UserLoginModel DecryptLoginModelFromToken(string token, IConfiguration _configuration)
        {
            UserLoginModel login_model = new UserLoginModel();
            try
            {
                var key = Get_AESKey(ConvertBase64StringToByte(_configuration["Setting:AESKey"]));
                var iv = Get_AESIV(ConvertBase64StringToByte(_configuration["Setting:AESIV"]));
