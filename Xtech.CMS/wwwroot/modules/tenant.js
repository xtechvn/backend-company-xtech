$(document).ready(function () {
    ...
    $('#token-input-role').tokenInput('/Role/GetRoleSuggestionList', {
        ...
        tokenLimit: 10,
        onAdd: function (item) {
            _tenant.OnChangeRoleId($(this).val());
        },
...
$("#ip-kup-search-user").keyup(function (e) {
    if (e.which === 13) {
        _tenant.OnChangeUser(e.target.value);
    } else {
...
var _tenant = {
    ...
    OnOpenCreateForm: function () {
        ...
        _tenant.modal_element.find('.modal-title').html(title);
        _tenant.modal_element.find('.modal-dialog').css('max-width', '900px');
        _ajax_caller.get(url, { Id: 0 }, function (result) {
            _tenant.modal_element.find('.modal-body').html(result);
            ...
            _tenant.modal_element.modal('show');
        });
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.isSuccess) {
                        _msgalert.success(result.message);
                        _tenant.ReLoad();
                        _tenant.modal_element.modal('hide');
                    } else {
                        _msgalert.error(result.message);
                    }
                },
    ...
    OnResetPassword: function (id) {
        ...
        _msgconfirm.openDialog(title, description, function () {
            $.ajax({
                url: "/Tenant/ResetPasswordByUserId",
                type: "post",
                data: { userId: id },
