...
{
    public class ArticleAPIRepository : IArticleAPIRepository
    {
        private readonly ArticleAPIDAL _ArticleDAL;
        private readonly TagAPIDAL _TagDAL;
        private readonly GroupProductDAL _groupProductDAL;

        private readonly string _UrlStaticImage;

        public ArticleAPIRepository(IOptions<DataBaseConfig> dataBaseConfig, IOptions<DomainConfig> domainConfig)
        {
            var _StrConnection = dataBaseConfig.Value.SqlServer.ConnectionString;
            _UrlStaticImage = domainConfig.Value.ImageStatic;
            _ArticleDAL = new ArticleAPIDAL(_StrConnection);
            _TagDAL = new TagAPIDAL(_StrConnection);
        ...
        public async Task<ArticleModel> GetArticleDetail(long Id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - GetArticleDetail: " + ex);
                return null;
            }
        }
        ...
        public async Task<long> SaveArticle(ArticleModel model, string url_upload, string key_encrypt)
        {
            try
            {
                ...
                var TBody = StringHelpers.ReplaceEditorImage(model.Body, _UrlStaticImage, key_encrypt);

                // upload thumb image via api
                var T11 = UpLoadHelper.UploadBase64Src(model.Image11, _UrlStaticImage, key_encrypt);
                var T43 = UpLoadHelper.UploadBase64Src(model.Image43, _UrlStaticImage, key_encrypt);
                var T169 = UpLoadHelper.UploadBase64Src(model.Image169, _UrlStaticImage, key_encrypt);
        ...
        public async Task<List<ArticleFeModel>> getArticleListByCategoryId(int cate_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - getArticleListByCategoryId: " + ex);
                return null;
            }
        }
        ...
        public async Task<ArticleFeDetailModel> GetArticleDetailLite(long article_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - GetArticleDetailLite: " + ex);
                return null;
            }
        }
        public async Task<ArticleFeModel> GetArticleDetailLiteFE(long article_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - GetArticleDetailiteFE: " + ex);
            }
            return null;
        }
        public async Task<ArticleFeModel> GetMostViewedArticle(long article_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - GetMostViewedArticle: " + ex);
            }
            return null;
        }
        public async Task<List<ArticleRelationModel>> FindArticleByTitle(string title, int parent_cate_faq_id)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - FindArticleByTitle: " + ex);
                return null;
            }
        }
        ...
        public async Task<ArticleFEModelPagnition> getArticleListByCategoryIdOrderByDate(int cate_id, int skip, int take, string category_name)
        {
            ...
            {
                LogHelper.InsertLogTelegram("[API] ArticleRepository - getArticleListByCategoryIdOrderByDate: " + ex);
                return null;
            }
        }
