    ...
    public class CustomerManagerRepository : ICustomerManagerRepository
    {
        ...
        public int SetUpClient(CustomerManagerView model)
        {
            try
            {
                ...
                {
                    var _ClientDAL = new ClientDAL(_configuration["DataBaseConfig:SqlServer:ConnectionString"]);
                    var data = _UserAgentDAL.GetUserAgentByClientId((int)model.Id);
                    var data2 = _ClientDAL.GetClientByID((int)model.Id).Result;
                    if (data != null)
                    {
        ...
        public async Task<CustomerManagerViewModel> GetDetailClient(long ClientId)
        {
            ...
            try
            {
                ...
                DataTable dt = await _ClientDAL.getClientById(ClientId);
        ...
        public int ResetStatusAc(long clientId, long Status, int type)
        {
            try
            {
                ...
                var model = _AccountClientDAL.GetAccountClientByClientId(clientId);
