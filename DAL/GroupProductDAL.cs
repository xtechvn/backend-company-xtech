    ...
    public class GroupProductDAL : GenericService<GroupProduct>
    {
        ...
        public async Task<List<GroupProduct>> getAllGroupProduct()
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var group_product = _DbContext.GroupProducts.AsNoTracking().Where(s => s.Status == 0).ToListAsync();
        ...
        public async Task<GroupProduct> getDetailByPath(string path)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var group_product = await _DbContext.GroupProducts.AsNoTracking().FirstOrDefaultAsync(s => s.Status == 0 && s.Path == path.Trim());
        ...
        public async Task<bool> IsGroupHeader(List<int> groups)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var group_product = await _DbContext.GroupProducts.AsNoTracking().Where(s => s.Status == 0 && groups.Contains(s.Id) && s.IsShowFooter==true).ToListAsync();
