...
namespace DAL
{
    public class UserDAL : GenericService<User>
    {
        ...
        /// <returns></returns>
        public async Task UpdateUserRoles(int userId, int[] arrayRole, int type = 0)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
            ...
            {
                LogHelper.InsertLogTelegram("UpdateUserRoles - UserDAL: " + ex);
            }

        }

        public async Task<List<int>> GetUserRolesId(int userId)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
        ...
        public List<User> GetListUserByRole(int role_id)
        {
            try
            {
                ...
                {
                    var user_role = _DbContext.UserRoles.Where(s => s.RoleId == role_id).ToList();
                    var UserRolesIds = user_role.Select(n => n.UserId).ToList();
                    if (UserRolesIds.Count > 0)
                    {
                        return _DbContext.Users.Where(s => UserRolesIds.Contains(s.Id) && s.Status == 0).ToList();
                    }
                }
            }
            catch (Exception ex)
